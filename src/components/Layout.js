            ...
            <div className="flex items-center justify-between h-16">
              ...
              <nav className="hidden md:flex items-center gap-10 text-sm font-semibold text-slate-700">
                ...
                {Object.entries(SERVICES).map(([service, items]) => (
                  <div key={service} className="relative group pb-3 -mb-3">
                    <span className="cursor-pointer text-slate-700 hover:text-slate-900">
                      {service}
                    </span>

                    <div className="absolute left-0 top-full hidden group-hover:block pt-3 -mt-3">
                      <div className="rounded-xl bg-white border border-slate-200 shadow-xl min-w-[260px]">
                        <ul className="py-2">
                          {items.map((item) => (
                            <li key={item.href}>
                              <Link
